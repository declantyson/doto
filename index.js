...
const updateBackground = (endpoint, var_name) => {
        ...
        if(typeof sourceData.preview === "undefined" || sourceData.preview.images.length === 0) {
            tryDownload(reasons.CANNOT_DETERMINE);
        } else {
            let resolutions = sourceData.preview.images[0].resolutions;
            if (resolutions.length < 0) {
                tryDownload(reasons.CANNOT_DETERMINE);
            } else if (resolutions[0].height > resolutions[0].width) {
                tryDownload(reasons.INCORRECT_ORIENTATION);
            } else {
                let urlComponents = source.split('.'),
                    format = urlComponents[urlComponents.length - 1];

                if(allowedFormats.indexOf(format) === -1) {
                    tryDownload(reasons.DISALLOWED_FORMAT);
                } else {
                    if(format === "gif") {
                        ...
                        request(source, (err, response, body) => {
                            ...
                            if(sourceEl === null) {
                                tryDownload(reasons.SOMETHING_BAD);
                            } else {
